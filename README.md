...
$$
S_{t+1} = S_t + \varepsilon_t, \qquad \varepsilon_t \sim \mathcal{N}(0, \sigma^2)
$$

A further decision was made to implement the finite horizon version of the Avellandeda-Stoikov model which includes a forced flat close out at ending time $T$ (Section 2.2). This choice was made over the alternative infinite horizon formulation in which a discount rate is used to discount the value of future outcomes indefinitely (Section 2.3) rather than providing a fixed end to the trading session.

The trading mechanism was a further distinction that this project adjusted from the Avellaneda – Stoikov paper. The paper includes both a "frozen inventory" passive agent who holds a fixed position and doesn't trade and an active agent posting bid and ask quotes that are filled according to a distance dependant Poisson intensity governed by the parameter $k$.
...
$$
\text{skewed mid}_t = \text{mid}_t - \gamma \cdot \text{inventory}_t
$$
...
$$
\gamma_{\text{project}} \equiv \gamma \cdot \sigma^2 \cdot (T-t)
$$
